...
Windows 11 passed installed-wheel qualification with Python 3.11-3.13. Windows
10 is an intended preview target but was not directly observed in the current
run.
...
No. `0.2.1.dev4` is unsigned and must be installed manually after checksum
verification.
